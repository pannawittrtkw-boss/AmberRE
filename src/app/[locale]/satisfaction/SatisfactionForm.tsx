...
export default function SatisfactionForm({ locale, ts }: { locale: string; ts: any }) {
  ...
  if (status === "loading") {
    return <div className="bg-white rounded-xl shadow-sm border p-8 mb-8" />;
  }

  if (!session) {
    return (
      <div className="bg-white rounded-xl shadow-sm border p-8 mb-8 text-center">
        <p className="font-semibold text-gray-800 mb-1">{ts.loginRequiredTitle}</p>
        <p className="text-gray-500 text-sm mb-4">{ts.loginRequiredNote}</p>
        <Link
          href={`/${locale}/auth/login`}
          className="inline-block bg-[#C8A951] text-white px-5 py-2 rounded-lg text-sm hover:bg-[#B8993F]"
        >
          {ts.loginCta}
        </Link>
      </div>
