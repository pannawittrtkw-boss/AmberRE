...
interface ContractInfo {
  ...
  unitNumber: string;
  signerName: string;
  role: "lessor" | "lessee" | "joint_lessee" | "commission";
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-sm w-full text-center space-y-4">
          ...
          <p className="text-sm text-gray-600">
            {info?.role === "lessor"
              ? "Lessor"
              : info?.role === "joint_lessee"
              ? "Joint Lessee"
              : info?.role === "commission"
              ? "Commission Agreement"
              : "Lessee"} · {info?.signerName}
          </p>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            <p className="text-xs text-amber-700 font-medium mb-0.5">
              {info?.role === "lessor"
                ? "Lessor (ผู้ให้เช่า)"
                : info?.role === "joint_lessee"
                ? "Joint Lessee (ผู้เช่าร่วม)"
                : info?.role === "commission"
                ? "Commission Agreement (ผู้ว่าจ้าง)"
                : "Lessee (ผู้เช่า)"}
            </p>
